...
After creating the table, provide a training paces section containing a markdown table with the columns: Pace Name, Pace (in the runner's preferred units), RPE, Heart Rate Range, and Purpose. Omit the Heart Rate Range column entirely if the runner does not train with a heart-rate monitor. Include easy, long run, marathon, threshold/tempo, interval, repetition, and goal race pace. State in one sentence which input the paces were derived from, and if the runner does not train with a GPS watch, add one sentence explaining how to run the plan by effort and time instead. If the pace is covered in this block do not add it in the schedule markdown table, for example if there's an easy run it doesn't need to have easy (pace listed next to it).
